...
export function CalculatorPrefeituraDaycoval({
  setAllInputsFilled,
  setFinalResult,
  banco,
}: {
  ...
  const params = JSON.parse(paramsString);
    ...
    let coeficiente84xDAYCOVAL = +params?.PREFEITURA?.["DAYCOVAL"]?.coeficiente_84x;
      ...
      setTotais([
        `TOTAL: R$ ${formatNumber(value / coeficiente84xDAYCOVAL)}`,
        `PARCELA R$ ${value}`,
        "84x",
      ]);

    const finalResult: string[] = [
      `Valor Empréstimo R$ ${formatNumber(value / coeficiente84xDAYCOVAL)}`,
      `VALOR TOTAL R$ ${formatNumber(value / coeficiente84xDAYCOVAL)}`,
